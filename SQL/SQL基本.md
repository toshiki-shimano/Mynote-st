...
```sql
select * from 口座
where 口座番号 like '2______' and
名義 like 'エ%' and 名義 like '%コ'
--口座番号が2000000代で、名義がエから始まり、コで終わる
```
...
```sql
-- 口座テーブルと廃止口座テーブルの口座番号、残高、解約時残高を一緒に取得する。そのうち口座テーブルは、残高が０のもの
-- 廃止の方は、残高が０ではないものを出す
...
```sql
-- 廃止講座テーブルに存在しない、口座の名義を知りたい。出力結果を降順にする
select 名義 from 口座
except
select 名義 from 廃止口座
order by 名義 desc
...
```sql
-- キャンペーンの価格が間違っていたので、3000を引いて、０．３％を残高に上乗せしたい
update 口座
set 残高 = (残高 - 3000) * 1.003
where 口座番号 in ('0652281','1026413', '2239710')
-- いちいち名前を書かなくてもinでまとめて記述できる
```
